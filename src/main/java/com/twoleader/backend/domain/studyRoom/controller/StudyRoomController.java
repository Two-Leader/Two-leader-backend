...
public class StudyRoomController {
  ...
  public String getStudyRoom(Model model, @PathVariable("room_uuid") UUID room_uuid,@PathVariable("user_uuid") UUID user_uuid) {
    GetStudyRoomResponse studyRoom = studyRoomService.findStudyRoomByRoom_uuid(room_uuid);
    GetUserResponse user = userService.getUser(GetUserRequest.builder().user_uuid(user_uuid).build());
