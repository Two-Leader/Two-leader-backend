...
public class WebRtcService {
  ...
  public void join(UUID roomUuid, UUID userUuid) {
    ...
    //        users.forEach(roomUser -> kurentoService.removeIncomingEndpoint(roomUser.getId(),
    // userId));
    //        kurentoService.removeIncomingEndpoint(userId);
    //        kurentoService.removeOutgoingEndpoint(userId);

    kurentoService.createOutgoingEndpoint(
        roomUuid,
        userUuid,
        event -> {
          ...
          stompMessagingService.sendToUser(
              userUuid,
              new OutputMessage<>(
                  WEBRTC_SUCCESS_ADD_ICE_CANDIDATE, webRtcMapper.toDto(userUuid, iceCandidate)));
        });
    ...
    stompMessagingService.sendToUser(
        userUuid, new OutputMessage<>(WEBRTC_SUCCESS_GET_STUDYROOM_USERS, users));
  }
  ...
  public void offer(UUID roomUuid, UUID userUuid, UUID senderUuid, String sdpOffer) {
    ...
    WebRtcEndpoint incomingEndpoint =
        userUuid == senderUuid
            ? kurentoService.getOutgoingEndpoint(userUuid)
            : kurentoService.createIncomingEndpoint(
                roomUuid,
                userUuid,
                senderUuid,
                event -> {
                  ...
                  stompMessagingService.sendToUser(
                      ...
                      new OutputMessage<>(
                          ...
                          webRtcMapper.toDto(senderUuid, iceCandidate)));
                });
    ...
    stompMessagingService.sendToUser(
        userUuid,
        new OutputMessage<>(WEBRTC_SUCCESS_ANSWER, webRtcMapper.toDto(senderUuid, sdpAnswer)));
