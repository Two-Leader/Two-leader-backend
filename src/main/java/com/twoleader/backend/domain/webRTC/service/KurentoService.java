...
public class KurentoService {
    ...
    private final ConcurrentMap<UUID, ConcurrentMap<UUID, WebRtcEndpoint>> incomingEndpoints = new ConcurrentHashMap<>();

    public void createOutgoingEndpoint(UUID roomUuid,UUID userUuid,
    ...
    public void removeIncomingEndpoint(UUID userUuid, UUID senderUuid) {
        if (incomingEndpoints.containsKey(userUuid) && incomingEndpoints.get(userUuid).containsKey(senderUuid)) {
    ...
    public void removeIncomingEndpoint(UUID userUuid) {
        if (incomingEndpoints.containsKey(userUuid)) {
            incomingEndpoints.remove(userUuid)
