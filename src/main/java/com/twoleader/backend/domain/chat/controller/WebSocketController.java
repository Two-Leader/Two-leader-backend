...
public class WebSocketController {
  ...
  @GetMapping
  public ResponseEntity getChat(@RequestParam("roomUuid") UUID roomUuid, @RequestParam(value = "page",defaultValue = "0") int page){
    Page<GetChatResponse> response = chatService.getChat(roomUuid, PageRequest.of(page,20));
    return ResponseEntity.ok(chatMapper.toModel(API_SUCCESS_CHAT_GET_ALL,response,roomUuid,page));
  }
