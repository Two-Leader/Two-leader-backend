...
public class ChatServiceTest {
    ...
    class getChatTest{
        ...
        public void whenSuccess(){
            ...
            for(int i = 0; i < 20; i++){
                chatList.add(Chat.builder().id(String.valueOf(i)).userId((long) i).roomUuid(roomUuid1).message("testMessage"+i).createdAt(LocalDateTime.now()).build());
            }
//            Page<Chat> chats = ;
            given(chatRepository.findAllByRoomUuid(any(),any())).willReturn(new PageImpl<>(chatList));

            Page<GetChatResponse> responses = chatService.getChat(UUID.randomUUID(), PageRequest.of(0,20));
